...
const SuperAdminSidebar = ({ isMobileOpen = false, onMobileClose = () => {} }) => {
  ...
  return (
    <>
      {/* Overlay: covers page content when sidebar is open on small screens. It intentionally sits below the navbar (navbar uses z-50). */}
      {isMobileOpen && (
        <div onClick={onMobileClose} className="fixed inset-0 z-30 lg:hidden bg-black/40" />
      )}

      <aside
        aria-hidden={!isMobileOpen && 'true'}
        className={`fixed left-0 lg:top-30 top-20 bottom-0 z-30 lg:z-40 bg-slate-900 text-white shadow-xl flex flex-col min-h-0 transform transition-transform duration-200 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} ${collapsed ? 'w-20' : 'w-64'} overflow-hidden`}
      >
        {/* Mobile close button */}
        {isMobileOpen && (
          <div className="p-2 lg:hidden flex justify-end">
            <button onClick={onMobileClose} aria-label="Close sidebar" className="p-2 rounded-md hover:bg-slate-800 text-white">
        ...
        <div className="p-4 flex items-center justify-between border-b border-slate-800">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-linear-to-r from-blue-500 to-purple-600 rounded-md flex items-center justify-center font-bold">S</div>
  ...
  <nav className="flex-1 overflow-y-auto hide-scrollbar px-2 py-3 min-h-0">
        <ul className="space-y-1">
          {items.map((it) => {
            const Icon = it.icon;
            return (
              <li key={it.path}>
                <NavLink to={it.path} className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-linear-to-r from-blue-600 to-purple-600 text-white' : 'text-slate-300 hover:bg-slate-700 hover:text-white'}`}>
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-linear-to-r from-green-400 to-blue-500 flex items-center justify-center text-white font-bold">{user?.name?.[0] || 'S'}</div>
